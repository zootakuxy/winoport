...
function header ( req:e.Request ): HeaderOpts {
    ...
    let port = Number( _origin.length===2? _origin[1]: req.protocol === "http"? 80 : 443 );
...
function nextHandler ( req:e.Request, res:e.Response, next:e.NextFunction, entry:DomainEntry, opts:HeaderOpts ){
    ...
    let protocol:string;
    let port:number;
    if( Number.isSafeInteger(entry[req.protocol])){
        port = entry[req.protocol];
        protocol = req.protocol;
    } else {
        port = entry.port;
        protocol = entry.protocol||req.protocol;
    }
